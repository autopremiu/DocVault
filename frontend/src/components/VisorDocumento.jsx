...
// Visor universal — Google Docs Viewer + Microsoft Office Online
// El archivo se conserva 100% en su formato original en MEGA
// Solo se usa el link público para abrirlo en el visor
...
export default function VisorDocumento({ uuid, nombre, tipo, onClose }) {
  ...
  const soportaOffice = ['docx','doc','xlsx','xls','xlsm','pptx','ppt'].includes(ext);
        ...
        <div style={s.body}>
          ...
          {estado === 'ok' && viewerUrl && (
            <>
              <iframe
                ...
                title={nombre}
              />
              {/* Tip si el iframe tarda en cargar */}
              <div style={s.tip}>
                ¿No carga el documento?{' '}
                <span style={{color:'#4f7cff',cursor:'pointer'}} onClick={recargar}>Reintentar</span>
                {' · '}
                <span style={{color:'#4f7cff',cursor:'pointer'}} onClick={()=>window.open(viewerUrl,'_blank')}>
                  Abrir en nueva pestaña
                </span>
                {' · '}
                <span style={{color:'#4f7cff',cursor:'pointer'}} onClick={descargar}>
                  Descargar archivo
                </span>
              </div>
            </>
          ...
          {estado === 'nolink' && (
            <div style={s.center}>
              ...
              <div style={s.cSub}>
                Este documento fue subido antes de que se generaran links públicos automáticamente.
...
const s = {
  ...
  toggle:     { display:'flex', background:'#0a0c10', borderRadius:8, border:'1px solid #1e2330', overflow:'hidden' },
  toggleBtn:  { padding:'5px 14px', border:'none', background:'none', color:'#6b7592', cursor:'pointer', fontSize:12, fontFamily:"'DM Sans',sans-serif", whiteSpace:'nowrap' },
  toggleActive:{ background:'rgba(79,124,255,.2)', color:'#7aa7ff' },
